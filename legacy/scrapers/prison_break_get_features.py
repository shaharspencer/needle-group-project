...
OUTPUT_JSON = "prisonbreak_characters_features.json"
OUTPUT_CSV = "prisonbreak_characters_features.csv"

# Initialize cloudscraper to bypass Cloudflare anti-bot checks
...
def get_all_character_urls(start_url):
    ...
    print("[+] Collecting character URLs from Prison Break wiki category pages...")
    ...
    while next_url:
        ...
        # Extract character links from the category member list
        members = soup.find_all("a", class_="category-page__member-link")
        for a in members:
            href = a.get("href", "")
            # Ensure it is a valid wiki page link and not a category/namespace page
        ...
        if infobox:
            ...
            for item in items:
                ...
                if label and val:
                    ...
                    text_val = re.sub(r"\[\d+\]", "", text_val).strip()  # Clean footnote references
