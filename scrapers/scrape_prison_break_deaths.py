...
URL = "https://listofdeaths.fandom.com/wiki/Prison_Break"
OUTPUT_CSV = "prison_break_deaths.csv"
...
def scrape_prison_break_deaths():
    ...
    for heading in content.find_all(["h2", "h3", "h4"]):
        ...
        if heading.name in ["h3", "h4"]:
            ...
            # Collect element siblings until the next section heading
            ...
            while curr and curr.name not in ["h2", "h3", "h4"]:
                ...
                for el in elements:
                    ...
                    if total_match:
                        ...
                        # Add victim line if it isn't the Total summary line
                        victims.append(clean_text)

                curr = curr.find_next_sibling()

            # Fallback to list count if no explicit Total line was found
            if not found_total_line:
                total_deaths = len(victims)

            # Extract episode identifier (e.g., "1x01", "S1E1", "Episode 1")
